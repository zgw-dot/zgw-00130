...
| 4. 导入（两种方式） | 方式A：点击「📥 按日期导入待核验」<br/>方式B：调用 CSV 导入接口 | 两种方式走**同一套统一导入流程**：冲突检测 → 去重 → 批量入库 |
| 5. 冲突检测 | 导入时自动检测「同患者同半天」冲突 | 同一患者同半天（同日期+同时段）跨号源预约，只有第一条能入库，其余会被拦截并列入 `conflicts` 数组 |
| 6. 逐项核验 | 逐条点击「编辑核验」，勾选化验/影像/知情/禁食四项，写备注后保存 | 必填项开关可在「⚙️ 规则配置」中调整 |
| 7. 材料不全时冻结 | 点击某行「冻结」→ 填写冻结原因（必填，例如「CT报告未出」）→ 确认 | 记录变 `frozen`，写入 `precheck_notifications` 冻结通知，写入审计 |
| 8. 补齐材料后放行 | 点击「放行」（普通放行）或「强放」（管理员特批） | 普通放行需所有必填核验项完成；强制放行绕过校验并写入 `force_released` 状态 |
| 9. 错误操作后撤销 | 对 `released`/`force_released` 状态的记录点击「撤销」→ 填写撤销原因 → 确认 | 状态回滚到 `revoked`，再次通知患者，全程审计 |
| 10. 导出对账 | 点击「📤 导出当前筛选」生成 CSV，到「📤 导出历史」页查看每次导出的快照哈希 | 哈希用于验证两次导出的一致性（见失败路径 3） |

### 统一导入流程

按日期导入和 CSV 导入共享同一套核心逻辑（`processImportBatch`），确保两种方式的冲突检测、去重规则、入库结果完全一致：

1. **已取消预约** → 直接跳过
2. **已导入过的预约**（同 appointment_id）→ 直接跳过
3. **同半天冲突**（同 patient_id + 同日期 + 同时段）→ 拦截，记入 `conflicts` 数组
4. **通过校验** → 写入 `precheck_records` 表，状态 `pending`
...
```json
{
  "imported": 6,
  "skipped": 1,
  "conflicts": [{ "patientName": "小华", "date": "2026-06-16", "period": "morning", "existingDoctor": "李医生", "reason": "..." }],
  "batchNo": "PRE202606156681",
  "total": 7,
  "message": "..."
}
```
